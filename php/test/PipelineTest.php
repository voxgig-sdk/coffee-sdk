...
{
    private static function utility(): CoffeeUtility
    {
        return new CoffeeUtility();
    }
    ...
    private static function ctx(array $over = []): CoffeeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CoffeeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CoffeeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CoffeeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CoffeeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CoffeeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CoffeeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CoffeeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CoffeeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CoffeeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CoffeeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CoffeeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CoffeeResponse([]),
            'result' => new CoffeeResult([]),
        ]);
        [, $err] = CoffeeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CoffeeSpec(['step' => 's']),
            'response' => new CoffeeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CoffeeResult([]),
        ]);
        [, $err] = CoffeeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CoffeeSpec(['step' => 's']),
            'response' => new CoffeeResponse(self::resp(200, ['v' => 1])),
            'result' => new CoffeeResult([]),
        ]);
        [, $err] = CoffeeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CoffeeSpec(['step' => 's']),
            'response' => new CoffeeResponse(self::resp(200, ['v' => 2])),
            'result' => new CoffeeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CoffeeMakeResponse::call($ctx);
    ...
    {
        $preset = new CoffeeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CoffeeSpec([]),
            'response' => new CoffeeResponse([]),
            'result' => new CoffeeResult([]),
        ...
        [$got, $err] = CoffeeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CoffeeResult([])]);
        [, $err] = CoffeeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CoffeeSpec([]), 'result' => null]);
        [, $err] = CoffeeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CoffeeSpec(['step' => 's']),
            'result' => new CoffeeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CoffeeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CoffeeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CoffeeSpec(['step' => 's']),
            'result' => new CoffeeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CoffeeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CoffeeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CoffeeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CoffeeSpec([]),
            'result' => new CoffeeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CoffeeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CoffeeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CoffeeError('boom', 'boom');
        $utility->fetcher = function (CoffeeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CoffeeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CoffeeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CoffeeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CoffeeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CoffeeResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CoffeeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CoffeeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CoffeeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CoffeeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CoffeeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CoffeeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CoffeeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CoffeeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CoffeeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CoffeeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CoffeeResult(['ok' => false])]);
        $this->expectException(CoffeeError::class);
        CoffeeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CoffeeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CoffeeMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CoffeeBaseFeature();
        $b = new CoffeeBaseFeature();
        CoffeeFeatureAdd::call($ctx, $a);
        CoffeeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CoffeeBaseFeature
    {
        $f = new CoffeeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CoffeeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CoffeeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CoffeeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CoffeePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CoffeeResponse(['headers' => null]),
            'result' => new CoffeeResult([]),
        ]);
        CoffeeResultHeaders::call($ctx);
