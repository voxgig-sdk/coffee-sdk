...
class HotEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new HotEntity($this->_client, $opts);
    }

    /**
     * @param Hot|array $args Hot data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CoffeeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Hot|array The current Hot data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Hot fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CoffeeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Hot fields).
     */
    ...
    /**
     * List Hot items matching the given filter.
     *
     * @param HotListMatch|array|null $reqmatch Match filter (any subset
     *   of Hot fields) as an assoc-array; HotListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Hot[]|array A list of Hot items as assoc-arrays at
     *   the SDK boundary; throws CoffeeError on failure (item-5 convention).
     */
