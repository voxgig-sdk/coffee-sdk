...
class CoffeeSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CoffeeHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CoffeeHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CoffeeFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CoffeeFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CoffeeUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CoffeeHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CoffeeHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CoffeeHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CoffeeError($op . "_allow",
                "CoffeeSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CoffeeHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CoffeeError("graphql_error",
                "CoffeeSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Iced($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_iced === null) {
                $this->_iced = new IcedEntity($this, null);
            }
            return $this->_iced;
        }
        return new IcedEntity($this, $data);
    }
