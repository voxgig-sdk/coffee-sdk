...
class CoffeeMakeError
{
    public static function call(?CoffeeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CoffeeContext([], null);
        }
        $op = $ctx->op ?? new CoffeeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CoffeeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CoffeeError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CoffeeError('', $msg, $ctx);
