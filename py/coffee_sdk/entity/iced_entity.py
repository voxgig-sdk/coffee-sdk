...
from coffee_sdk.coffee_types import (
    Iced,
    IcedListMatch,
)
...
class IcedEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return IcedEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Iced]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Iced().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
