...
MCP server exposing the Coffee SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `coffee_list` | `entity`, optional `query` map | First page of records as JSON |
| `coffee_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `coffee_list` and
`coffee_load` tools become available in new sessions.
