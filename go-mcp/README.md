...
[MCP](https://modelcontextprotocol.io) server exposing the Coffee SDK as
two agent tools — `coffee_list` and `coffee_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/coffee-mcp)
make build

# 2. Provide credentials via the environment
export COFFEE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user coffee \
  -- /absolute/path/to/coffee-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./coffee-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// coffee_list: first page of records
{ "entity": "hot" }
{ "entity": "hot", "query": { } }

// coffee_load: one record by id
{ "entity": "hot", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/coffee-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user coffee \
     -- "$PWD"/dist/*/coffee-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `coffee_list` and `coffee_load` tools now appear
   in new sessions. Ask the agent to *"list hot using coffee"*
   and it calls `coffee_list` with `{"entity":"hot"}`.
...
```sh
export COFFEE_APIKEY=sk_live_xxx            # API key
export COFFEE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `coffee_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `coffee_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `COFFEE_APIKEY` | API key sent with every request. |
| `COFFEE_BASE` | Optional override of the API base URL. |
...
```sh
./coffee-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"coffee_load","arguments":{"entity":"hot","query":{"id":1}}}}'
```
