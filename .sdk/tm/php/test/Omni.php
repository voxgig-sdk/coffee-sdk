...
//    only - a typed CoffeeContext there would make every ctx
...
final class CoffeeOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CoffeeContext($clean, null);
    ...
    public static function err_from_map($m): ?CoffeeError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CoffeeError(is_string($code) ? $code : '', $msg);
    }
