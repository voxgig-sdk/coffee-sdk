...
class FtCtrl extends CoffeeControl
{
...
class FtClient
{
    ...
    public function __construct(CoffeeUtility $utility, array $options)
    {
    ...
    public function get_utility(): CoffeeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CoffeeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CoffeeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CoffeeConfig::make_feature($name);
    ...
    private static function buildUrl(CoffeeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CoffeeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CoffeeResponse($fetched);
        }
    ...
    private function fail(CoffeeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CoffeeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
