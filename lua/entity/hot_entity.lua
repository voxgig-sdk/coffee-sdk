...
local HotEntity = {}
HotEntity.__index = HotEntity


function HotEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HotEntity)
  self._name = "hot"
  self._client = client
...
function HotEntity:get_name()
  return self._name
end


function HotEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HotEntity.new(self._client, opts)
end
...
function HotEntity:mark_deleted()
  self._deleted = true
end


function HotEntity:deleted()
  return true == self._deleted
end


function HotEntity:data_set(args)
  if args ~= nil then
...
function HotEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HotEntity:match_set(args)
  if args ~= nil then
...
function HotEntity:stream(action, args, callopts)
...
---@param reqmatch HotListMatch
---@param ctrl? table
---@return Hot[]
---@return string? err
function HotEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function HotEntity:_run_op(ctx, post_done)
  local utility = self._utility
