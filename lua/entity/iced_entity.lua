...
local IcedEntity = {}
IcedEntity.__index = IcedEntity


function IcedEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IcedEntity)
  self._name = "iced"
  self._client = client
...
function IcedEntity:get_name()
  return self._name
end


function IcedEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IcedEntity.new(self._client, opts)
end
...
function IcedEntity:mark_deleted()
  self._deleted = true
end


function IcedEntity:deleted()
  return true == self._deleted
end


function IcedEntity:data_set(args)
  if args ~= nil then
...
function IcedEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IcedEntity:match_set(args)
  if args ~= nil then
...
function IcedEntity:stream(action, args, callopts)
...
function IcedEntity:_run_op(ctx, post_done)
  local utility = self._utility
