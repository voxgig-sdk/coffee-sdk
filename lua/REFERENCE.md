# Coffee Lua SDK Reference

Complete API reference for the Coffee Lua SDK.
...
```lua
local results, err = client:Hot(nil):list(nil, nil)
```
...
```lua
local results, err = client:Iced(nil):list(nil, nil)
```
