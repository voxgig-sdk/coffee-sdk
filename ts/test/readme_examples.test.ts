...
//     (\`new CoffeeSDK(...)\` / \`CoffeeSDK.test(...)\` become
//     \`CoffeeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"hot":{"test01":{"id":"test01"}},"iced":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CoffeeSDK(...)\` and any
// \`CoffeeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CoffeeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CoffeeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
