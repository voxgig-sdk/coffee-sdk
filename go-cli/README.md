...
boru-driven command-line client **and** interactive REPL for the Coffee
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/coffee-cli)
make build
...
./coffee-cli list hot
./coffee-cli list iced

# 5. Override the API base URL for a single call
COFFEE_BASE=https://api.example.com ./coffee-cli list hot
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/coffee-cli
   ```
...
Run with no arguments to open a REPL (prompt `coffee>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `hot`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/coffee-cli`. |
...
(`./coffee-cli <expr>`) and interactively (the REPL), and expressions compose the
