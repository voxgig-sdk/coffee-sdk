// Command Coffee-cli is a CLI + REPL for the Coffee SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      COFFEE_APIKEY (api key), COFFEE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: COFFEE_APIKEY carries the API key and
	// COFFEE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("COFFEE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("COFFEE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCoffeeSDK(opts)
