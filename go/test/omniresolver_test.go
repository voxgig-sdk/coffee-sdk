...
import (
	...
	omni "github.com/voxgig-sdk/coffee-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CoffeeError); is {
				out := map[string]any{
					"name":    "CoffeeError",
...
func omniCtx(arg any, client *sdk.CoffeeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CoffeeSDK); is {
			client = live
