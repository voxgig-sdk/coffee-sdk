...
func TestHotEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Hot(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Hot(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		hotRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.hot", setup.data)))
		...
		hotRef01Match := map[string]any{}

		hotRef01ListResult, err := hotRef01Ent.List(hotRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, hotRef01ListOk := hotRef01ListResult.([]any)
		if !hotRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", hotRef01ListResult)
		}
...
func hotBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COFFEE_TEST_HOT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COFFEE_TEST_HOT_ENTID"])
