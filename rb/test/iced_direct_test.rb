...
class IcedDirectTest < Minitest::Test
  def test_direct_list_iced
    setup = iced_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-iced", setup[:live] ? "live" : "unit")
    if _should_skip
...
def iced_direct_setup(mockres)
  ...
  if live
    ...
    client = CoffeeSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
