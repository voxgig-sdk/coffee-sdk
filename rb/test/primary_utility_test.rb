...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = CoffeeOmni.make_runner(TEST_JSON_FILE, CoffeeSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : CoffeeOmni.errify(err)
    end
