...
class HotEntity
  ...
  def make
    opts = @_entopts.dup
    HotEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CoffeeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Hot, Hash] the current Hot data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CoffeeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Hot fields)
  def match_get
  ...
  #
  # @param reqmatch [HotListMatch, Hash, nil] match filter (any subset of
  #   Hot fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Hot>, Array] the matching Hot items; raises CoffeeError on failure
